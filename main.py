...
async def command_start(message: Message):
    ...
    if user:  # Если что-то есть
        # (1, 84779542, None, None, None, None, None)
...
async def start_register(message: Message):
    chat_id = message.chat.id
    await Register.full_name.set()  # Ща будет вопрос
...
async def get_full_name_ask_gender(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id, 'Выберите свой пол: ', reply_markup=generate_genders_buttons())
...
async def get_age_ask_phone(message: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id, 'Введите телефон в формате +998123456789: ')
